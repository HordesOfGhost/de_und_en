...
from services.db.models import ListeningMetaData  

def get_prompt_for_generating_listening_content(level:str, db:Session):

  recent_topics = db.query(ListeningMetaData.topic)\
                      .filter(ListeningMetaData.level == level)\
                      .order_by(ListeningMetaData.id.desc())\
                      .limit(11)\
                      .all()
...
def get_listening_task_evaluation_prompt(topic, article, questions_and_answers, user_answers):
  prompt = "You are an expert evaluator for a German listening comprehension task.\n\n"
  prompt += "Topic: " + topic + "\n\n"
  prompt += "Article:\n" + article + "\n\n"
